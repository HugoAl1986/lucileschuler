...
export class HttpHorseService {
  ...
  constructor(private http: HttpClient, private behaviourService:BehaviourService, private utils:UtilsService) { 
  ...
  createHorse(horse: any, idClient: string) {
    return this.http.post(this.url + `admin/create_horse/${idClient}`, horse).pipe(
      tap((horse: Horse) => {
        const horses = this.horses.getValue();
        const clients = this.clients.getValue();
        const indexClient = _.findIndex(clients, (client:Client) => client.id ==idClient);
  ...
  removeHorse(id: string) {
    return this.http.delete(this.url + `admin/remove_horse/${id}`).pipe(
      tap((data) => {
        ...
        _.remove(horses, (horse: Horse) => {return horse.id == id});
        ...
        _.forEach(clients, (client:Client, index) => {
          _.remove(clients[index].horses,(horse:Horse) => {return horse.id == id})
        })
        this.clients.next(clients);
      })
    );
    ...
    return this.http
      ...
      .pipe(
        tap((horse: Horse) => {
          ...
          const horses = this.horses.getValue();
          const indexHorse = _.findIndex(horses,(horse: Horse) => horse.id == id);
          _.update(horses, `[${indexHorse}]`, (hors: Horse) => {return horseFromForm});
          this.horses.next(horses);

          //update this.clients

          const clients = this.clients.getValue();
          const indexClient = _.findIndex(clients,(client: Client) => client.id == horseFromForm.client.id);
          const indexHorseFromClient = _.findIndex(clients[indexClient].horses,(horse: Horse) => horse.id == id);
          _.update(clients,`[${indexClient}].horses[${indexHorseFromClient}]`,(horse: Horse) => {return horseFromForm});
          this.clients.next(clients);
        })
      );
  }
